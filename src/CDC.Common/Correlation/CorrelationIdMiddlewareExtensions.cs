...
/// <summary>
/// Registers <see cref="CorrelationIdMiddlewareExtensions"/>'s middleware. Shared by CDC.Api and
/// CDC.Web so a single request/response across both services carries the same correlation ID.
/// </summary>
public static class CorrelationIdMiddlewareExtensions
{
    ...
    // Inbound IDs come from outside the trust boundary (a browser for CDC.Web, or CDC.Web itself
    // for CDC.Api), so only a well-formed GUID is trusted; anything else is replaced rather than
    // logged verbatim.
